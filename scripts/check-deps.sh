...
show_install_commands() {
    ...
    case $pm in
        0) # Arch Linux
            ...
            echo "sudo pacman -S hyprland xdg-desktop-portal xdg-desktop-portal-hyprland waybar swww hyprlock rofi-wayland dunst"
            echo "sudo pacman -S kitty dolphin firefox"
            echo "sudo pacman -S networkmanager network-manager-applet"
            echo "sudo pacman -S wireplumber brightnessctl"
            echo "sudo pacman -S cliphist wl-paste"
            echo ""
            ...
            echo ""
            echo "# AUR packages:"
            echo "yay -S hyprshot matugen-bin pwvucontrol"
            echo ""
            echo "# Optional:"
        ...
        1) # Debian/Ubuntu
            ...
            echo "sudo apt install wl-paste cliphist"
            echo "sudo apt install brightnessctl"
            echo "# Note: pwvucontrol may need to be installed from source or Flatpak"
        ...
        2) # Fedora
            ...
            echo "sudo dnf install hyprland xdg-desktop-portal xdg-desktop-portal-hyprland waybar swww hyprlock"
            echo "sudo dnf install rofi dunst kitty dolphin firefox"
            echo "sudo dnf install NetworkManager-applet"
            echo "sudo dnf install wl-paste cliphist"
            echo "sudo dnf install brightnessctl blueman"
            echo "# Note: pwvucontrol may need to be installed from source or Flatpak"
        ...
        *) # Unknown
            ...
            echo "- hyprland, xdg-desktop-portal, xdg-desktop-portal-hyprland, waybar, swww, hyprlock, rofi, dunst"
            echo "- kitty, dolphin, firefox"
            echo "- network-manager-applet, brightnessctl"
            echo "- pwvucontrol (PipeWire volume control - may need manual install)"
            echo "- wl-paste, cliphist"
...
main() {
    ...
    check_command "pwvucontrol" "Audio control (PipeWire)" "false"
    check_command "brightnessctl" "Brightness control" "false"
    check_command "hyprshot" "Screenshot tool" "false"
    check_command "wl-paste" "Clipboard tool" "false"
    check_command "cliphist" "Clipboard history tool" "false"
    # XDG Desktop Portal don't have commands
    echo -e "${BLUE}XDG Desktop Portal${NC}"
    echo "-------------------"
    echo "XDG Desktop Portal is not a command, it's a service"
    echo "It's installed by default on most systems"
    echo "If it's not installed, you can install it with your package manager"
    echo "For example, on Arch Linux, you can install it with:"
    echo "sudo pacman -S xdg-desktop-portal xdg-desktop-portal-hyprland"
    echo
    # Theming tools
